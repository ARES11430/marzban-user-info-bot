...
export const getUserInfo = async (username: string, adminID?: number): Promise<IUserInfo> => {
	const query = `SELECT
  u.id,
  u.username,
  u.status,
  ...
  LIMIT 1`;
	...
	const [rows] = await pool.query<IUserInfo[]>(query, adminID ? [username, adminID] : [username]);
...
export const getLowTrafficUsers = async (traffic: number, adminID?: number) => {
  ...
  `;
	...
	const [rows] = await pool.query<IUserTrafficLimit[]>(query, [trafficInBytes, adminID]);
...
export const getExpiringUsers = async (adminID?: number): Promise<IExpiringUsersResult> => {
