...
const loadConfig = async () => {
     ...
     try {
          ...
          console.log(`No existing config found at ${configFilePath}, creating default config`);
...
export const addDatabaseAdmin = async (admin: DatabaseAdmin) => {
     ...
     if (!config.dataBaseAdmins.some((existingAdmin: DatabaseAdmin) => existingAdmin.id === admin.id)) {
...
export const removeDatabaseAdmin = async (id: number) => {
     const config = await loadConfig();
     config.dataBaseAdmins = config.dataBaseAdmins.filter((admin: DatabaseAdmin) => admin.id !== id);
     await saveConfig(config);
};
...
export const removeTelegramAdminID = async (telegramID: number) => {
     const config = await loadConfig();
     config.telegramAdmins = config.telegramAdmins.filter((id: number) => id !== telegramID);
     await saveConfig(config);
};
...
export const sendLongMessage = async (ctx: Context, text: string, maxLength = 4096) => {
